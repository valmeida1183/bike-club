...
export class AuthService {
	...
	signIn(email: string, password: string) {
		const signIn$ = this.http.post<IAuthResponseData>(
			this.signInUrl,
			new AuthRequestData(email, password),
		);
		const user$ = signIn$.pipe(
			switchMap((authResponseData) =>
				this.getUser(authResponseData.email, authResponseData.idToken),
			),
		);
	...
	autoSignIn() {
		...
		const authUserData = new AuthUserData(
		...
		if (authUserData.currentToken) {
			...
			this.getUser(authUserData.email, authUserData.currentToken).subscribe(
				(user) => {
	...
	private handleAuthentication(authResponseData: IAuthResponseData, user: User) {
		...
		const expirationDate = new Date(
			currentTime + +authResponseData.expiresIn * 1000,
		);
		const authUserData = new AuthUserData(
	...
	private createUser(user: User, token: string) {
		this.http
			.post(this.userUrl, user, { params: new HttpParams().set('auth', token) })
			.subscribe((response) => {
	...
	private getUser(email: string, token: string) {
		return this.http
			.get<User>(`${this.userUrl}?orderBy="email"&equalTo="${email}"`, {
