...
The project uses a hybrid approach, leveraging both modern NgRx Signals and classic NgRx (Store).
...
-   **Classic NgRx (Store)**:
    -   **Shopping List**: Manages the list of bikes loaded from the API, including actions (`loadBikes`, `loadBikesSuccess`, `loadBikesFailed`) and selectors for accessing the `shoppingList` state slice.
...
1.  `requestParamsInterceptor`: Adds common query parameters to requests.
...
-   **Service Structure**: API interaction logic resides in dedicated services (e.g., `ShopApiService` in `src/app/features/shopping/services/`).
...
1.  Dispatch the `loadBikes` action with any required query parameters.
2.  Subscribe to the relevant selector, e.g., `store.select(state => state.shoppingList.bikes)`.
3.  Implement logic to handle `loadBikesSuccess` and `loadBikesFailed` cases.
...
Branch `folder-refactoring` - Indicates ongoing structural improvements to codebase organization.
